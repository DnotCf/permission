...
<script type="text/javascript">
    $(function () {
        ...
        function loadRoleList() {
            $.ajax({
                url: "<%=request.getContextPath() %>/sys/role/list.json",
        ...
        function bindRoleClick() {
            ...
            $(".role-delete").click(function (e) {
                ...
                if (confirm("确定要删除角色[" + deptName + "]吗?")) {
                    $.ajax({
                        url: "<%=request.getContextPath() %>/sys/role/delete.json",
                        data: {
                            id: deptId
                        },
                        success: function (result) {
        ...
        function loadRoleAcl(selectedRoleId) {
            ...
            $.ajax({
                url: "<%=request.getContextPath() %>/sys/role/roleTree.json",
                data : {
        ...
        function recursivePrepareTreeData(aclModuleList) {
            ...
            if (aclModuleList && aclModuleList.length > 0) {
                $(aclModuleList).each(function(i, aclModule) {
                    ...
                    if (aclModule.aclDtos && aclModule.aclDtos.length > 0) {
                        $(aclModule.aclDtos).each(function(i, acl) {
                            zTreeObj.push({
                                id: aclPrefix + acl.id,
                    ...
                    if ((aclModule.aclModuleList && aclModule.aclModuleList.length > 0) ||
                        (aclModule.aclDtos&& aclModule.aclDtos.length > 0)) {
        ...
        $(".saveRoleAcl").click(function (e) {
            ...
            $.ajax({
                url: "<%=request.getContextPath() %>/sys/role/changeAcls.json",
                data: {
                    roleId: lastRoleId,
        ...
        function loadRoleUser(selectedRoleId) {
            $.ajax({
                url: "<%=request.getContextPath() %>/sys/role/users.json",
                data: {
                    roleId: selectedRoleId
                },
                type: 'POST',
        ...
        $(".saveRoleUser").click(function (e) {
            ...
            $.ajax({
                url: "<%=request.getContextPath() %>/sys/role/changeUsers.json",
                data: {
                    roleId: lastRoleId,
